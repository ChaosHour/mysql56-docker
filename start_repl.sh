...
OUTPUT=$(docker exec -it mysql56-docker-primary-1 mysql -u root -ps3cr3t -e "FLUSH TABLES; FLUSH TABLES WITH READ LOCK; SHOW MASTER STATUS; SELECT SLEEP(1); UNLOCK TABLES")
...
docker exec -it mysql56-docker-replica-1 mysql -u root -ps3cr3t -e "STOP SLAVE;CHANGE MASTER TO MASTER_HOST='172.22.0.3',MASTER_USER='repl',MASTER_PASSWORD='slAv3',MASTER_LOG_FILE='${MASTER_LOG_FILE}',MASTER_LOG_POS=${MASTER_LOG_POS};SELECT SLEEP (10);START SLAVE"
